...
void CImageCouleur::sauvegarde(const std::string file)
{
    ...
    if (this->m_ppucPixel)
    {
        std::string nomFichier = "res/"; // force sauvegarde dans r�pertoire res (doit exister)
...
// organis� en vecteur progressif � savoir 3*i -> R ou H,3*i+1 -> G ou S et 3*i+2 -> B ou V
...
// gestion des plans
CImageNdg CImageCouleur::plan(int choix, double poidsRouge, double poidsVert, double poidsBleu)
{ // 0 -> R ou H, 1 -> V ou S, 2 -> B ou V et 3 -> luminance d'o� les poids fct de l'illuminant
    ...
    case 3:
        for (int i = 0; i < this->lireNbPixels(); i++)
            out(i) = (unsigned char)(poidsRouge * (double)this->operator()(i)[0] +
                                     poidsVert * (double)this->operator()(i)[1] +
                                     poidsBleu * (double)this->operator()(i)[2]);
    }
    return out;
}

// mise � jour d'un plan particulier d�fini par une image en Ndg et 0,1 ou 2
    ...
    if (methode.compare("HSV") == 0)
    {
        ...
        for (int i = 0; i < this->lireNbPixels(); i++)
        {
            ...
            if (rgb_max != rgb_min)
            {
                ...
                else if (rgb_max == this->operator()(i)[1])
                {
