...
std::vector<SIGNATURE_Forme> CImageClasse::sigComposantesConnexes(bool enregistrementCSV)
{
    ...
    if (this->lireNbRegions() > 0)
    {
        ...
        if (enregistrementCSV)
        {
            ...
            {
                f << "Objet; CG_i; CG_j; Surface; codeFreeman ; RE_Hi ; RE_Hj ; RE_Bi ; RE_Bj ; perimetre ; lambda1 ; "
                ...
                for (int k = 1; k < (int)tab.size(); k++)
                    f << k << ";" << tab[k].centreGravite_i << ";" << tab[k].centreGravite_j << ";" << tab[k].surface
                      ...
                      << " ; " << tab[k].lambda1 << " ; " << tab[k].lambda2 << " ; " << tab[k].V1i << " ; "
                      << tab[k].V1j << " ; " << tab[k].V2i << " ; " << tab[k].V2j << " ; " << std::endl;
            }
            f.close();
        }
    }
...
std::vector<SIGNATURE_Cellule> CImageClasse::sigCellules(const CImageNdg & img, bool enregistrementCSV)
{
    ...
    if (this->lireNbRegions() > 0)
    {
        ...
        {
            tab[k].emergence =
                (ngObj[k] / tab[k].occupation) - (tab[k].emergence / (tab[k].surface - tab[k].occupation));
...
void CImageClasse::circleInEachComponent(const std::vector<SIGNATURE_Forme> & tab)
{
    const int circleColor = this->lireNbRegions() + 1;
    double    theta_step  = M_PI / 180;
    ...
    for (int num = 1; num <= this->lireNbRegions(); num++)
    {
        ...
        // Find max and min distance from center to edges
        for (int i = tab[num].rectEnglob_Hi; i <= tab[num].rectEnglob_Bi; i++)
        {
            for (int j = tab[num].rectEnglob_Hj; j <= tab[num].rectEnglob_Bj; j++)
            {
                if (this->operator()(i, j) == static_cast<unsigned long>(num) and isEdgePixel(i, j, num))
                {
                    double distance = sqrt(pow(i - centerX, 2) + pow(j - centerY, 2));
                    maxRadius       = std::max(maxRadius, distance);
                    minRadius       = std::min(minRadius, distance);
                }
            }
        }

        // Draw circles
        drawCircle(centerX, centerY, static_cast<int>(minRadius), circleColor);
        drawCircle(centerX, centerY, static_cast<int>(maxRadius), circleColor);
    }
}

bool CImageClasse::isEdgePixel(int i, int j, int num)
{
    for (int dx = -1; dx <= 1; dx++)
    {
        for (int dy = -1; dy <= 1; dy++)
        {
            int ni = i + dx;
            int nj = j + dy;
            // Check bounds and whether the neighbor belongs to the shape
...
void CImageClasse::drawCircle(int p_cx, int p_cy, int p_radius, int p_color)
{
    const double theta_step = M_PI / 180;

    for (double theta = 0; theta < 2 * M_PI; theta += theta_step)
    {
        int x = p_cx + p_radius * cos(theta);
        int y = p_cy + p_radius * sin(theta);
