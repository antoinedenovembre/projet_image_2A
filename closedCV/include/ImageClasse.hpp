...
public : 
	...
	_EXPORT_ CImageClasse(const CImageClasse& in, std::string choix = "sans", std::string voisinage = "V8"); // re-etiquetage eventuel
	_EXPORT_ CImageClasse(const CImageNdg& im, int nbClusters = 2, std::string choix = "aleatoire"); // clustering 
	_EXPORT_ CImageClasse(const CImageCouleur& im, int nbClusters = 2, std::string choix = "aleatoire", int plan=0); // plan dans HSV, non dans espace 3D
	_EXPORT_ CImageClasse(const CImageNdg& im, const std::vector<int>& germes, double tolerance = 5); // croissange region avec ensemble de germes sous la forme x0,y0,x1,y1,x2,y2 etc
	...
	_EXPORT_ CImageCouleur affichage(const std::vector<SIGNATURE_Couleur>& tab, const std::string& methode = "moyenne");
	...
	_EXPORT_ std::vector<SIGNATURE_Forme> sigComposantesConnexes(bool enregistrementCSV = false);

	// cellules de Voronoe
	_EXPORT_ CImageClasse cellules(); // cellules de Voronoe
