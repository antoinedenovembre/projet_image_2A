...
public:
	...
	_EXPORT_ CImageCouleur(int hauteur, int largeur, int valR = -1, int valV = -1, int valB = -1); // si -1 alors non pixels non initialis�s
	...
	_EXPORT_ CImageCouleur(const CImageNdg& im, const CImageNdg& binaire, int R = 255, int G = 0, int B = 0); // img binaire de pr�f�rence contours
	_EXPORT_ CImageCouleur(const CImageCouleur& im, const CImageNdg& binaire, int R = 255, int G = 0, int B = 0); // img binaire de pr�f�rence contours
	...
	_EXPORT_ CImageNdg plan(int choix = 3, double poidsRouge = 1. / 3., double poidsVert = 1. / 3., double poidsBleu = 1. / 3.); 
	...
	_EXPORT_ CImageCouleur transformation(const std::string methode = "expansion"); // version avec sortie sur [0,255] pour expansion notamment sur chaque plan
