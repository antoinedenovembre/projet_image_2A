...
private:
	...
	int              m_iHauteur;                                   // hauteur de l'image
	int              m_iLargeur;                                   // largeur de l'image
	string      m_sNom;                                       // nom de l'image
	unsigned char*   m_pucData;                                    // données de l'image
	unsigned char**  m_ppucPixel;                                  // pointeurs sur les pixels de l'image
...
public:
	...
	_declspec(dllexport) CImageCouleur(int hauteur, int largeur, int valR = -1, int valV = -1, int valB = -1); // si -1 alors pixels non initialisés
	...
	_declspec(dllexport) CImageCouleur(const CImageNdg& pR, const CImageNdg& pG, const CImageNdg& pB);   //constructeur à partir de 3 plans
	...
	// pouvoir accéder à un pixel par image(i,j)[plan]                             //attention opere sur un plan particulier
	...
	_declspec(dllexport) CImageNdg plan(int choix = 3, double poidsRouge = 1. / 3., double poidsVert = 1. / 3., double poidsBleu = 1. / 3.);  //par défaut poids égaux à 1/3
