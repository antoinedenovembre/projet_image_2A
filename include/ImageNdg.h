...
typedef unsigned short WORD;    //alias de type WORD pour les entiers non signés sur 2 octets(16 bits)
typedef unsigned int DWORD;	 //alias de type DWORD pour les entiers non signés sur 4 octets(32 bits)
typedef unsigned long LONG;  //alias de type LONG pour les entiers non signés sur 4 octets(32 bits)

typedef struct tagBITMAPFILEHEADER     //création d'une structure pour l'entête du fichier BMP
{
	WORD  bfType;
	DWORD bfSize;         //ce champs stocke la taille du fichier en octets
	WORD  bfReserved1;     //ces champs sont réservés pour des usages futurs
	WORD  bfReserved2;
	DWORD bfOffBits;        //Ce champ indique le décalage en octets, à partir du début du fichier, où commencent les données de l'image(bitmap)
} BITMAPFILEHEADER;

typedef struct tagBITMAPINFOHEADER     //pour l'entête d'information du fichier BMP
                                       //cette structure contient des informations sur les dimensions et le nombre de couleurs utilisées par le bitmap
{
	DWORD biSize;   //stocke la taille de cette structure en octets
	LONG  biWidth;  //largeur de l'image en pixels
	LONG  biHeight; //hauteur de l'image en pixels, champs négatif si l'image est orientée vers le haut
	WORD  biPlanes; //nombre de plans de couleurs utilisés pour stocker les données de l'image, toujours 1
	WORD  biBitCount; //nombre de bits utilisés pour coder la couleur de chaque pixel, 1, 4, 8, 16, 24 ou 32
	DWORD biCompression; //type de compression utilisé pour stocker les données de l'image, 0 si pas de compression
	DWORD biSizeImage;  //taille en octets des données de l'image, 0 si pas de compression
	LONG  biXPelsPerMeter; //résolution horizontale en pixels par mètre du périphérique cible, 0 si pas de compression
	LONG  biYPelsPerMeter;  //résolution verticale en pixels par mètre du périphérique cible, 0 si pas de compression
	DWORD biClrUsed;   //nombre de couleurs utilisées dans l'image, 0 si pas de compression
	DWORD biClrImportant;    //nombre de couleurs importantes pour l'image, 0 si pas de compression
} BITMAPINFOHEADER;
